...
def init_problem(N):
    masses=np.random.uniform(0,mass_max, size=(2,N))
    positions_x=np.random.uniform(x_min,x_max, size=(N))
    positions_y=np.random.uniform(y_min,y_max, size=(N))
...
# votre code
def force_unique(i, j, masses, positions, G):
    return G*masses[i]*masses[j]*((1/np.sqrt((positions[0][i]-positions[0][j])**2+(positions[1][i]-positions[1][j])**2))**3)*(positions[:,j]-positions[:,i])
    ...
    for i in range (N):
        for j in range(N) : 
            if j==i:
                ...
                forces[0][i]+=force_unique(i,j,masses, positions, G)[0]
                forces[1][i]+=force_unique(i,j,masses, positions, G)[1]
    return forces
    """
...
def simulate(masses, positions, speeds, dt=0.1, nb_steps=100):
    N=len(masses)
    positions_across_time=np.zeros((nb_steps, 2, N))
    for i in range(nb_steps):
        positions_across_time[i] = positions
        F = forces(masses, positions)
        a = F/masses
        speeds = speeds + a*dt
        positions = positions + speeds*dt
    return positions_across_time
...
def draw(simulation, masses, colors=None, scale=10.):
    ...
    for i in range(N):
        plt.scatter(simulation[:, 0, i], simulation[:, 1, i],
                     c=[colors[i]])
