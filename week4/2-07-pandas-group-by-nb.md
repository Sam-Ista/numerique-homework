...
```{code-cell} ipython3
print (len(df['Pclass'].unique())*len(df['Sex'].unique())*len(df['Survived'].unique()))
```
...
```{code-cell} ipython3
df2=df.groupby(['Pclass', 'Sex', 'Survived']).size()
```
...
```{code-cell} ipython3
df4=df.groupby('Sex')
df4.get_group('female').groupby('Pclass').get_group(1).groupby('Survived').get_group(0)
```
...
```{code-cell} ipython3
mask=(df['Sex']=='female') & (df['Survived']==0) & (df['Pclass']==1)
df[mask]
```
...
```{code-cell} ipython3
dict={}
df3=df.groupby(['Sex','Pclass'])
for i in df['Sex'].unique() :
    for j in df['Pclass'].unique() : 
        dict[(i,j)]=df3['Survived'].value_counts(normalize=True)[i, j, 1]
print (dict)
```
...
```{code-cell} ipython3
se=pd.Series(dict,name='taux de survie par genre dans chaque classe')
se
```
