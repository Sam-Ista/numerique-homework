...
```{code-cell} ipython3
df['Mass 2']=pd.to_numeric(df['Mass (lb) orig'].str.replace(">", "").str.replace("<", ""))
df['Mass 3']=df['Mass (lb) orig'].str.replace(">", "").str.replace("<", "").astype(int)
df.dtypes
```
...
```{code-cell} ipython3
df['Mass (kg)']=(df['Mass 2']/2.205).astype(int)
print(df['Mass (kg)'], df['Mass 2'])
```
