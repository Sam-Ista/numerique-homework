...
```{code-cell} ipython3
df2=pd.read_csv('petit-titanic.csv', delimiter=';', header= None, names=['PassengerId', 'Survived', 'Pclass', 'Name', 'Sex', 'Age', 'SibSp', 'Parch', 'Ticket','Fare', 'Cabin', 'Embarked'])
df2.head(2)
```
...
```{code-cell} ipython3
df2.set_index('Identifiant', inplace= True)
```
